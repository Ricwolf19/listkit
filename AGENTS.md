...
- **Dark mode is class-driven and additive.** Every literal color utility carries a `dark:` sibling on the surface ladder — page `gray-950`, containers/rows `gray-900`, floating panels and controls `gray-800`, hovers/selected one step lighter (`gray-700`) — and adding a dark class never changes the light style. Theme-object classes get their dark siblings inside `theme/colorTheme.ts`, not at call sites. `tailwind.css` ships that variant (`@custom-variant dark (&:where(.dark, .dark *))`), so a consumer opts in by toggling the class, not by declaring semantics — under Tailwind v4's default `dark:` means `prefers-color-scheme`, which rendered light-only apps' lists dark. Never drop it from `tailwind.css`. Backgrounds on pinned/sticky cells stay alpha-free in both modes (invariant 15).
...
8. **User view preferences persist.** Anything describing _how a user works with this list_ — column order/width/visibility, density, table-vs-cards, page size, the quick-filter bar — is stored through the column-prefs layer (localStorage by default, pluggable via `columnStorage`). A URL param always wins over the stored value, so a shared link shows the sender's view. The saved table-vs-cards choice carries one extra rule: the viewport outranks it, so a narrow screen opens on cards whatever was stored, and a toggle made there is never written back — a phone visit must not redefine how the list opens on a desktop. A new view-level toggle joins that record; a value that changes _which rows_ are shown belongs in the URL instead, never in storage.
...
12. **Everything that floats goes through the overlay primitives.** Dropdowns, popovers and menus render through `PopupPortal` + `useAnchoredPopup`; dialogs go through `components/overlays/Modal`. Hand-rolling an absolutely-positioned panel reintroduces the two bugs those primitives exist to kill: a parent with `overflow-hidden` clips it, and a right-aligned trigger pushes it off a narrow viewport. Dismissal is `useOutsideClick` + `useEscapeKey`, never a bespoke listener — `Modal` refcounts the scroll lock and keeps an open-stack so Escape closes the innermost overlay, and a second implementation would fight it. A `role='menu'` dropdown additionally joins `LIST_MENU_GROUP` (`useExclusiveMenu`, one list menu open at a time) and wires `useMenuNavigation` for the WAI-ARIA arrow-key pattern. Any scrollable region uses `ScrollArea`, whose `overscroll-behavior` is per-axis on purpose (a blanket `overscroll-contain` over an x-only scroller swallows the page's vertical wheel delta). Overlays occupy one tier and one tier only: an app shell's chrome sits at `z-80`/`z-90` (that is what `@pibytelabs/ui` uses), every overlay — `Modal`, `FilterSidebar`, the options bottom sheet — takes `z-100` so its scrim covers that chrome, and `PopupPortal` takes `z-110` so a select opened inside an overlay still paints over it. Within the tier a panel must out-rank its own scrim: raising a backdrop without raising the panel with it hands every tap to the backdrop, which is exactly how the options sheet became untappable on mobile.
...
- **Consumers can delete local `objectIdMatch`-style overrides** now that the
  aggregate executor casts `$match` through the schema (invariant 17).
