...
export type FetchAdapterConfig<T> = {
	url: string
	/** Map a ListQuery to query-string params. Defaults to page/pageSize/search/sort. */
	transformQuery?: (query: ListQuery) => Record<string, string>
	/** Map the parsed JSON response to { data, total }. Defaults to identity. */
	transformResponse?: (json: unknown) => ListResult<T>
	/** Extra fetch options (headers, credentials, …). */
 ...
 * `{ data, total }` JSON body (override via transformQuery/transformResponse).
 */
